       ...
       <div class="col-sm-3 col-md-2 sidebar">
          ...
          <ul class="nav nav-sidebar">
           ...
           <li><h4>&nbspДополнительные настройки</h4></li>
            ...
            <li class="active"><a href="departments.php">Кафедры</a></li>
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <h3 class="page-header" align="center">Список кафедр</h3>
...
if(isset($_POST['insert']))
  {
    ...
    $connect= pg_connect("host=localhost port=5432 dbname=sework_new user=postgres password=postgres");
      $res=pg_query($connect,"INSERT INTO departments (name_dep, abbreviation, date_create, not_valid) 
        VALUES ('$p1','$p2','$p3', '$p4');");
...
");
  ...
  if(isset($_GET['delete']))
  {
    $delete = $_GET['delete'];
    $connect= pg_connect("host=localhost port=5432 dbname=sework_new user=postgres password=postgres");
    $result_users = pg_query($connect,"DELETE FROM departments WHERE id ='$delete'");
  ...
  { 
    ...
    $connect= pg_connect("host=localhost port=5432 dbname=sework_new user=postgres password=postgres");
    $result_departments = pg_query($connect,"SELECT *FROM  departments WHERE id ='$edit'");
    ...
    $mass_departments = pg_fetch_row($result_departments);
      ...
      { 
        print '<div class="form-group">
          ...
          <div class="col-xs-9">
            <input type="text" name="'.$i.'" class="form-control" id="login" align="left" value="'.trim($mass_departments[$i]).'" >
          </div>
        </div>';
      }
...
if(isset($_GET['add']))
  {
    ...
    print '<form class="form-horizontal" method ="POST">';
   $arrayRow = array('0' => 'ID пользователя','1' => 'Название кафедры', '2' => 'Абривиатура',
     '3' => 'Дата создания', '4' => 'Дата отменны'  );
    for ($i=1; $i <= 2 ; $i++) 
      { 
        print '<div class="form-group">
...
<?php
  ...
  $result_departments = pg_query($connect,"SELECT  * FROM  departments ORDER BY id ");
    for ($b=0; $b <=4 ; $b++) 
    { 
       print '<td>'.trim($arrayRow[$b]).'</td>';
    }
    //print '<td> Добавить факультет</td>';
    print '<td  colspan="2"> <a href="departments.php?add=ok"><span class="glyphicon glyphicon-plus">Добавить</a></td>';
    print '<td  colspan="2"> <a href="departments.php?add=ok">Отменить: Да/Нет</a></td>';
  while ($mass_departments = pg_fetch_row($result_departments))
   {
      print '<tr>';
      for ($i=0; $i <= 4 ; $i++) 
      { 
        print '<td>'.trim($mass_departments[$i]).'</td>';
      }
      print '<td> <a href="departments.php?edit='.$mass_departments[0].'"><span class="glyphicon glyphicon-pencil  "></a></td>';
      print '<td> <a href="departments.php?delete='.$mass_departments[0].'"><span class="glyphicon glyphicon-remove"></a></td>';
      print '<td> <a href="departments.php?up='.$mass_departments[0].'"><span class="glyphicon glyphicon-thumbs-up"></a></td>';
      print '<td> <a href="departments.php?down='.$mass_departments[0].'"><span class="glyphicon glyphicon-thumbs-down"></a></td>';
